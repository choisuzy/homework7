...
int initialize(headNode** h); /* 이중포인터를 매개변수로 하여 headNode에 메모리를 할당하여 초기화하는 initialize 함수 정의 */ 
...
int initialize(headNode** h) { /* 이중포인터를 매개변수로 하여 headNode에 메모리를 할당하여 초기화하는 initialize 함수 정의 */ 
                               /* lab3에서는 값을 변경할 수 없고 이 함수는 값을 변경할 수 있음 */ 
	...
	*h = (headNode*)malloc(sizeof(headNode)); /* headNode크기만큼을 동적할당한 headNode포인터를 포인터h에 넣음 */
	(*h)->first = NULL; /* 포인터h의 first부분을 NULL로 만듦 */
	return 1; /* 1를 반환함 */
}
...
int insertLast(headNode* h, int key) { /* key에 대한 노드 하나를 추가하는 insertLast 함수 정의 */
	...
	while(n->rlink != NULL) { /* n의 rlink부분이 빈공간이 아니라면 */
		n = n->rlink; /* n에 n의 rlink부분을 넣음 */
	}
	n->rlink = node; /* n의 rlink부분에 node를 넣음 */
	node->llink = n; /* node의 llink부분에 n을 넣음 */
	return 0; /* 0을 반환함 */
}
...
int deleteLast(headNode* h) { /* list의 마지막 노드를 삭제하는 deleteLast 함수 정의 */
	...
	if(n->rlink == NULL) { /* n의 rlink부분이 빈공간이라면 */
	...
	/* 마지막 노드까지 이동 */
	while(n->rlink != NULL) { /* n의 rlink부분이 빈공간이라면 */
		trail = n; /* trail에 n을 넣음 */
		n = n->rlink; /* n에 n의 rlink부분을 넣음 */
	}

	/* n이 삭제되므로, 이전 노드의 링크 NULL 처리 */
	trail->rlink = NULL; /* trail의 rlink부분을 빈공간으로 만듦 */
	free(n); /* n을 free시킴 */
...
int insertFirst(headNode* h, int key) { /* list 처음에 key에 대한 노드 하나를 추가하는 insertFirst 함수 정의 */
	...
	listNode* node = (listNode*)malloc(sizeof(listNode)); /* listNode크기만큼을 동적할당한 listNode포인터를 listNode포인터 node에 넣음 */
	node->key = key; /* node의 key부분에 key값을 넣음 */
	node->rlink = node->llink = NULL; /* node의 rlink부분과 node의 llink부분을 NULL로 만듦 */
	...
	node->rlink = h->first; /* node의 rlink부분에 h의 first부분을 넣음 */
	node->llink = NULL; /* node의 llink부분을 빈공간으로 만듦 */

	listNode *p = h->first; /* listnode의 포인터p 부분에 h의 first부분을 넣음 */
	p->llink = node; /* p의 llink부분에 node을 넣음 */
	h->first = node; /* h의 first부분에 node을 넣음 */
...
int deleteFirst(headNode* h) { /* list의 첫번째 노드를 삭제하는 deleteFirst 함수 정의 */
	...
	listNode* n = h->first; /* listNode포인터 n에 h의 first부분을 넣음 */
	h->first = n->rlink; /* h의 first부분에 n의 rlink부분을 넣음 */
